...
import os
import logging
from ddpg_agent import DDPGAgent, ReplayBuffer, GruDDPGAgent
...
def train_gru_ddpg(env: ElectromagneticDamperEnv, agent: GruDDPGAgent, replay_buffer: Gru_ReplayBuffer, 
                   n_episodes=200, min_buffer_size=1000, print_interval=5, save_interval=5, 
                   save_path=None, rand_prob=0,
                   start_episode=0, initial_episode_rewards=None):
    """## 训练基于GRU的DDPG代理
    ...
    # 记录到日志
    logging.info(f"开始GRU DDPG训练 - {datetime.now()}")
    logging.info(f"总轮次: {n_episodes}, 起始轮次: {start_episode}")
    logging.info(f"序列长度: {agent.seq_len}")
    ...
    # 创建奖励日志文件
    rewards_log_file = os.path.join(os.path.dirname(save_path), f"gru_rewards_log{current_time}.csv") if save_path else None
    if rewards_log_file:
        with open(rewards_log_file, "w") as f:
            f.write("episode,reward,avg_reward,critic_loss,actor_loss,epsilon\n")
    ...
    for episode in tqdm(range(start_episode, n_episodes), desc="GRU DDPG训练轮次"):
        ...
        while not done:
            ...
            if len(replay_buffer) > min_buffer_size:
                try:
                    ...
                    print(f"更新GRU网络时发生错误: {e}")
                    logging.error(f"更新GRU网络时发生错误: {e}")
                    raise e
        ...
        if checkpoints_path and (episode + 1) % save_interval == 0:
            # 保存完整检查点 (包含所有训练状态)
            checkpoint_name = f"gru_{current_time}_ep{episode+1}_checkpoint.pth"
            agent.model_name,_ = os.path.splitext(checkpoint_name)
            checkpoint_path = f"{checkpoints_path}/{checkpoint_name}"
    ...
    # 记录最终结果
    final_msg = f"GRU训练完成，已保存最终模型: {checkpoint_name}, 训练轮次: {episode+1}, 最终奖励: {float(episode_reward):.2f}, 平均奖励: {float(avg_reward):.2f}"
    print(final_msg)
    logging.info(final_msg)
    logging.info(f"最终平均奖励: {float(avg_rewards[-1]):.4f}")
