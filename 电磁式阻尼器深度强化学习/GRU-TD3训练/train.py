...
def train_td3(env: ElectromagneticDamperEnv, agent: Union[TD3Agent, Gru_TD3Agent], replay_buffer: Union[ReplayBuffer, Gru_ReplayBuffer], 
    ...
    for episode in tqdm(range(start_episode, n_episodes), desc="训练轮次"):
        ...
        while not done:
            ...
            if env.use_time_input: state = np.concatenate([state, np.array([train_datasets.dt_history[-1]])])
            action = float(agent.select_action(state, add_noise=True, epsilon=epsilon, rand_prob=rand_prob))
            ...
            # 存储经验
            if env.use_time_input: next_state = np.concatenate([next_state, np.array([train_datasets.dt_history[-1]])])
