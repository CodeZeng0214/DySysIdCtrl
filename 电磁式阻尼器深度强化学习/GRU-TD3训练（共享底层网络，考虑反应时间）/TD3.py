...
class BaseTD3Agent:
    ...
    def update(self, replay_buffer: Union[ReplayBuffer, Gru_ReplayBuffer]) -> Tuple[float, float, float]:
        ...
        # 更新参数
        if self.critic1.gru_predictor is not None: self.critic1.gru_predictor.unfreeze_gru()
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            if self.actor.gru_predictor is not None: self.actor.gru_predictor.freeze_gru()
...
class Gru_TD3Agent(BaseTD3Agent):
    ...
    def _init_nn(self):
        ...
        self.actor = Gru_Actor(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=self.hidden_dim, action_bound=self.action_bound,
                               gru_predictor=self.gru_predictor).to(device)

        self.critic1 = Gru_Critic(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=self.hidden_dim, 
                                  gru_predictor=self.gru_predictor).to(device)

        self.critic2 = Gru_Critic(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=self.hidden_dim, 
                                  gru_predictor=self.gru_predictor).to(device)

        # 目标网络使用目标GRU预测器
        self.target_actor = Gru_Actor(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=self.hidden_dim, action_bound=self.action_bound, gru_predictor=self.target_gru_predictor).to(device)

        self.target_critic1 = Gru_Critic(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=self.hidden_dim, 
                                         gru_predictor=self.target_gru_predictor).to(device)

        self.target_critic2 = Gru_Critic(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=self.hidden_dim,
                                         gru_predictor=self.target_gru_predictor).to(device)
